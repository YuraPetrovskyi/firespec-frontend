...
const CATEGORIES = [projInf, preIns, siteIns, postIns];
  ...
  return (
    <ProtectedLayout>
      {logs.length !== 0 ? (
        <div className="p-6 max-w-5xl mx-auto space-y-8 pb-20">
          ...
          {logs.map((log) => {
            ...
            const grouped = CATEGORIES.reduce(
              (acc: Record<string, Change[]>, category) => {
                acc[category] = parsedChanges.filter(
            ...
            return (
              <div
                key={log.id}
                className="bg-white shadow rounded-md p-5 border"
              >
                ...
                <div>
                  ...
                  {grouped[preIns].length > 0 ? (
                    ...
                    <p className="text-gray-400 text-sm mb-2 ml-2">
                  ...
                  {grouped[projInf].length > 0 ? (
                    <ul className="space-y-1 mb-2 bg-sky-500/5 p-2 rounded-md">
                      {grouped[projInf].map((c, i) => (
                        <li
                          key={i}
                          className="text-sm flex flex-col gap-2 ml-2"
                        >
                ...
                <div>
                  ...
                  {grouped[siteIns].length > 0 ? (
                    <div className="space-y-4 bg-sky-500/5 p-3 rounded-md">
                      ...
                      {Object.entries(
                        grouped[siteIns].reduce(
                          (acc: Record<string, Change[]>, change) => {
                            ...
                            {items.map(
                              (c, i) =>
                                c.field === "main" && (
                                  <li
                                    key={i}
                                    className="text-sm font-semibold flex flex-wrap gap-2"
                                  >
                                    <span className="text-gray-600 break-all">
                                      Main category:
                                    </span>
                    ...
                    <ul className="space-y-1 mb-4 bg-sky-500/5 p-2 rounded-md break-words">
                      {grouped[postIns].map((c, i) => (
                        <li key={i} className="text-sm flex flex-col gap-2">
                          ...
                          <span className="text-red-600 border-2 border-dashed ml-4 p-1">
                            {c.from ?? "---"}
                          </span>
                          <span className="text-green-600 border-2 border-dashed ml-4 p-1">
                            {c.to ?? "---"}
                          </span>
                        </li>
                      ))}
        ...
        <p className="p-8 text-lg text-center text-gray-500">
          No change logs found for this inspection.
        </p>
      )}
      ...
      {errorMessage && (
        <ErrorModal message={errorMessage} onClose={() => router.back()} />
      )}
    </ProtectedLayout>
  );
}
