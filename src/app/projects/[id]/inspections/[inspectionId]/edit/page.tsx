...
export default function EditInspectionPage() {
  ...
  const [oldProjectInformation, setOldProjectInformation] =
  ...
  const generateChangeLog = (
    ...
    inspectionSchema.postInspection.forEach(({ name, label }) => {
      compare(
        "Post-Inspection",
        name,
        oldData.postInspection?.[name],
        newData.postInspection?.[name],
        label
      );
    });
    ...
    Object.entries(newData.siteInspections).forEach(([category, values]) => {
      Object.entries(values).forEach(([field, val]) => {
        ...
        if (String(oldVal ?? "") !== String(val ?? "")) {
          const categorySchema = inspectionSchema.siteInspections.find(
            (c) => c.name === category
          );
          const categoryLabel = categorySchema?.label || category;
          const optionLabel =
            categorySchema?.options.find((o) => o.name === field)?.label ||
            field;
    ...
    <ProtectedLayout>
      ...
      {showOfflineSuccess && (
        <ModalConfirm
          ...
          message="Your changes have been saved locally and will automatically sync when you're back online."
          nameAction="Continue Editing"
          confirmColor="blue"
